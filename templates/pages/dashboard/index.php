...
?>
                            ...
                            <?php else: ?>
                                <p class="production-card__title"><?= htmlspecialchars($buildJob['label'] ?? $buildJob['building']) ?>
                                    • niveau <?= format_number($buildJob['targetLevel']) ?></p>
                                <?php if ($buildEndTime !== null): ?>
                                    <p class="production-card__time" data-countdown-container
                                       data-server-now="<?= $serverNow ?>" data-endtime="<?= (int)$buildEndTime ?>">
                                        Termine dans <span
                                                class="countdown"><?= htmlspecialchars(format_duration((int)($buildJob['remaining'] ?? 0))) ?></span>
                                    </p>
                                <?php else: ?>
                                    <p class="production-card__time">Termine
                                        dans <?= htmlspecialchars(format_duration((int)($buildJob['remaining'] ?? 0))) ?></p>
                                <?php endif; ?>
                            <?php endif; ?>
                            <footer class="production-card__footer">
                                <span><?= format_number($queues['buildings']['count'] ?? 0) ?> amélioration(s) en attente</span>
                                <a class="link-button"
                                   href="<?= htmlspecialchars($baseUrl) ?>/colony?planet=<?= $selectedPlanetId ?>">Ouvrir
                                    la colonie</a>
                            </footer>
                        </div>
                        <div class="production-card">
...
?>
                            ...
                            <?php else: ?>
                                <p class="production-card__title"><?= htmlspecialchars($researchJob['label'] ?? $researchJob['research']) ?>
                                    • niveau <?= format_number($researchJob['targetLevel'] ?? 0) ?></p>
                                <?php if ($researchEndTime !== null): ?>
                                    <p class="production-card__time" data-countdown-container
                                       data-server-now="<?= $serverNow ?>" data-endtime="<?= (int)$researchEndTime ?>">
                                        Termine dans <span
                                                class="countdown"><?= htmlspecialchars(format_duration((int)($researchJob['remaining'] ?? 0))) ?></span>
                                    </p>
                                <?php else: ?>
                                    <p class="production-card__time">Termine
                                        dans <?= htmlspecialchars(format_duration((int)($researchJob['remaining'] ?? 0))) ?></p>
                                <?php endif; ?>
                            <?php endif; ?>
                            <footer class="production-card__footer">
                                <span><?= format_number($queues['research']['count'] ?? 0) ?> programme(s) planifié(s)</span>
                                <a class="link-button"
                                   href="<?= htmlspecialchars($baseUrl) ?>/research?planet=<?= $selectedPlanetId ?>">Accéder
                                    au laboratoire</a>
                            </footer>
                        </div>
                        <div class="production-card">
...
?>
                            ...
                            <?php else: ?>
                                <p class="production-card__title"><?= htmlspecialchars($shipJob['label'] ?? $shipJob['ship']) ?>
                                    × <?= format_number($shipJob['quantity'] ?? 0) ?></p>
                                <?php if ($shipEndTime !== null): ?>
                                    <p class="production-card__time" data-countdown-container
                                       data-server-now="<?= $serverNow ?>" data-endtime="<?= (int)$shipEndTime ?>">
                                        Livraison dans <span
                                                class="countdown"><?= htmlspecialchars(format_duration((int)($shipJob['remaining'] ?? 0))) ?></span>
                                    </p>
                                <?php else: ?>
                                    <p class="production-card__time">Livraison
                                        dans <?= htmlspecialchars(format_duration((int)($shipJob['remaining'] ?? 0))) ?></p>
                                <?php endif; ?>
                            <?php endif; ?>
                            <footer class="production-card__footer">
                                <span><?= format_number($queues['shipyard']['count'] ?? 0) ?> commande(s) actives</span>
                                <a class="link-button"
                                   href="<?= htmlspecialchars($baseUrl) ?>/shipyard?planet=<?= $selectedPlanetId ?>">Accéder
                                    au chantier</a>
                            </footer>
                        </div>
                    </div>
                        ...
                        <?php if ($activePlanet): ?>
                            ...
                            <ul class="planet-summary__resources planet-summary__characteristics">
                                <li>
                                    ...
                                    <div class="planet-summary__resource-values">
                                        <strong><?= htmlspecialchars(format_number(max(0, $activePlanet->getDiameter()))) ?>
                                            km</strong>
                                    </div>
                                </li>
                                <li>
                                    ...
                                    <div class="planet-summary__resource-values">
                                        <strong><?= htmlspecialchars(format_number($activePlanet->getTemperatureMax())) ?>
                                            °C</strong>
                                    </div>
                                </li>
                                <li>
                                    ...
                                    <div class="planet-summary__resource-values">
                                        <strong><?= htmlspecialchars(format_number($activePlanet->getTemperatureMin())) ?>
                                            °C</strong>
                                    </div>
                                </li>
                            </ul>
                            ...
                            <div class="planet-summary__actions">
                                <a class="button button--primary"
                                   href="<?= htmlspecialchars($baseUrl) ?>/colony?planet=<?= $selectedPlanetId ?>">Gérer
                                    la colonie</a>
                                <a class="button button--ghost"
                                   href="<?= htmlspecialchars($baseUrl) ?>/research?planet=<?= $selectedPlanetId ?>">Programme
                                    scientifique</a>
                            </div>
                        <?php else: ?>
